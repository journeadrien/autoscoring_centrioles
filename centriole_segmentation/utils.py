...
def calculate_mAP(device,det_boxes, det_labels, det_scores, true_boxes, true_labels, true_difficulties,n_classes):
    """
...
def read_marker_txt(path,VALID_MARKER):
    f= open(op.join(path,'marker_name.txt'),"r")
    lines = f.readlines()
    f.close()
    channel_dict = {x.split(' : ')[1].replace('\n','') : x.split(' : ')[0]  for x in lines}
    valid_channel_dict = {}
    for valid_marker in VALID_MARKER:
        if valid_marker in channel_dict.keys():
            valid_channel_dict[valid_marker] = channel_dict[valid_marker] 
   
    return valid_channel_dict
