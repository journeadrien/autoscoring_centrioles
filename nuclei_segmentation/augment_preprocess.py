"""
Created on Fri Apr 10 14:25:18 2020
...
import h5py
from config import DATASET_PATH, load_img
...
def make_n_save_mosaic(train_path,csv_file):
    """
...
#    data_to_array()
    ...
    annot_path = op.join(DATASET_PATH,'external/annotations/')
    tissue_path = op.join(DATASET_PATH,'external/tissue_images/')
    train_path = op.join(DATASET_PATH,'stage1_train/')
    csv_path = op.join(DATASET_PATH,'share_train_df.csv')
    print(train_path)
    # preprocess_external_data(annot_path, tissue_path, train_path)

    make_n_save_mosaic(train_path,csv_path)
